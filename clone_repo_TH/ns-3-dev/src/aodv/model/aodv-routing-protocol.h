...
public:
  ...
  RoutingTable GetRoutingTable () const { return m_routingTable; }
  ...
  void OutputDataToFile(PacketTimeSentTag ptst_tag, Ptr<const Packet> p, bool learning_packet, TrafficType t,Ipv4Address i);
  ...
  */
  ...
  /// Receive and process control packets without actually reading it from the socket...
  void RecvAodvWithoutRead (Ptr<Socket> socket, Ptr<Packet> packet, Address sourceAddress);

  Neighbors* GetNeighbors() {return &m_nb;}
  /* below 2 attempts didnt work & give weird errors ...
   Neighbors GetNeighbors() { return m_nb; }
   Ptr<Neighbors> GetNeighbors() { return Create<Neighbors>(m_nb);}
  */

  /// Statistics
  int QStatistics() { return m_qlrn_packets_sent; }
  int AODVStatistics() { return m_control_packets_sent; }
  int TrafficStatistics() { return m_traffic_packets_sent; }
  int LrnTrafficStatistics() { return m_learning_packets_sent; }
  int RecvTrafficStatistics() { return m_traffic_packets_received; }
  int RecvTrafficStatistics(Ipv4Address i) { return m_traffic_packets_received_per_src[i]; }
  int RecvLrnTrafficStatistics() { return m_learning_packets_received; }
  std::string GetQStatistics();
  std::string GetAODVStatistics();
  std::string GetTrafficStatistics();
  std::string GetLrnTrafficStatistics();
  std::string GetRecvTrafficStatistics();
  std::string GetRecvLrnTrafficStatistics();
  // and more ?
  ...
  void CorrectPacketTrackingOutput(Ptr<const Packet> p, Ipv4Header = Ipv4Header()); //for when pkts are dropped due to non-existing routes
  void CheckTraffic(Ptr<const Packet> p, TrafficType& );
  /// detecting ICMP TTL Exc
  bool CheckIcmpTTLExceeded(Ptr<const Packet> p, Icmpv4TimeExceeded& ii);
  // void SetSrcAODV(Ptr<aodv::RoutingProtocol> i, Ipv4Address ip) { m_src_aodvProto[ip] = i; }

protected:
  virtual void DoInitialize (void);
private:
  /// keep the information about our peer from the network to check
  /// to get information about nr of packets sent
  // std::map<Ipv4Address,Ptr<aodv::RoutingProtocol> > m_src_aodvProto;

  /// Callbacks for tracing the packet Tx/Rx events
  TracedCallback<Ptr<const Packet> > m_txTrace;
  TracedCallback<Ptr<const Packet> > m_rxTrace;

  //Statistics
  void PacketTrackingInput(Ptr<const Packet> p, Ipv4Header header);
  //duplicate, also in qlearner
  bool CheckAODVHeader (Ptr<const Packet> p);

  unsigned int m_qlrn_packets_sent; //aodv control packets
  unsigned int m_control_packets_sent; //aodv control packets
  unsigned int m_traffic_packets_sent; //recognized as one of the types of traffic (web/voice/vid)
  unsigned int m_learning_packets_sent; //learning traffic packets
  unsigned int m_other_packets_sent; // ?
  unsigned int m_qlrn_packets_received;
  unsigned int m_control_packets_received; //aodv control packets
  unsigned int m_traffic_packets_received; //recognized as one of the types of traffic (web/voice/vid)
  std::map<Ipv4Address,unsigned int> m_traffic_packets_received_per_src;
  unsigned int m_learning_packets_received; //learning traffic packets
  unsigned int m_other_packets_received; // ?
