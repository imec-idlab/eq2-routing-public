...
public:
  ...
  */
  ...
  // TODO parameter for how much to reduce it by
  void ReduceAmountOfTraffic(uint64_t = 0, float = 0.0);
  void IncreaseAmountOfTraffic(uint64_t = 0, float = 0.0);
...
private:
  ...
  uint64_t        m_backup_maxBytes;     //!< Limit total number of bytes sent
  uint64_t        m_totBytes;     //!< Total bytes sent so far
  EventId         m_startStopEvent;     //!< Event id for next start or stop event
  EventId         m_sendEvent;    //!< Event id of pending "send packet" event
  TypeId          m_tid;          //!< Type of the socket used

  bool m_learning; //(thomas)
  int m_packet_number;
