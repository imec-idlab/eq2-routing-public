...
Icmpv4L4Protocol::SendMessage (Ptr<Packet> packet, Ipv4Address source, Ipv4Address dest, uint8_t type, uint8_t code, Ptr<Ipv4Route> route)
{
  ...
  NS_LOG_DEBUG( "echo reply : " << packet->GetUid());
  m_downTarget (packet, source, dest, PROT_NUMBER, route); //?N
}
void
                              ...
                              Ipv4Address destination)
{
  ...
  NS_LOG_DEBUG("current icmp seq nr:  " << echo.GetSequenceNumber() << "  packetId    " << p->GetUid() );
