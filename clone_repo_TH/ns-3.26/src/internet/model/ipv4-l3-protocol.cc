...
void Ipv4L3Protocol::SetDelay(int new_delay) {
  if (!uv) {
    ...
    uv->SetAttribute ("Max", DoubleValue(250000)); // +- 0.25 ms is the maximum the delay can vary
  }
  m_delay_ms = new_delay;
}

void Ipv4L3Protocol::SetLoss(float new_loss) {
  NS_ASSERT_MSG(new_loss <= 1.0, "setting packet loss to more than 100% is not supported." );
  if (!uv) {
    uv = CreateObject<UniformRandomVariable> ();
    uv->SetAttribute ("Min", DoubleValue(0));
    uv->SetAttribute ("Max", DoubleValue(100000));
  }
  m_packet_loss_param = new_loss;
}


int Ipv4L3Protocol::JitterDelayMs(int m_delay_ms) {
  return m_delay_ms * 1000000 + uv->GetValue() * (m_jitter ? 10:1 );
}
                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  NS_LOG_LOGIC ("Packet with id " << p->GetUid() << " from " << from << " received on node " <<
                m_node->GetId ());
  ...
  for (Ipv4InterfaceList::iterator ifaceIter = m_interfaces.begin ();
       ifaceIter != m_interfaces.end (); ifaceIter++, ifaceIndex++)
    {
      ...
      for (uint32_t j = 0; j < GetNAddresses (ifaceIndex); j++)
        {
          Ipv4InterfaceAddress ifAddr = GetAddress (ifaceIndex, j);
          NS_LOG_LOGIC ("Testing address " << ifAddr.GetLocal () << " with mask " << ifAddr.GetMask () << " " << packet->GetUid());
  ...
  if (route && route->GetGateway () != Ipv4Address ())
    {
      ...
      SendRealOut (route, packet->Copy (), ipHeader, m_delay_ms != 0); //here the delay will already have been applied
      return;
    }
  // 4) packet is not broadcast, and is passed in with a route entry but route->GetGateway is not set (e.g., on-demand)
  if (route && route->GetGateway () == Ipv4Address ())
  ...
  if (newRoute)
    {
      ...
      m_sendOutgoingTrace (ipHeader, packet, interface);
      SendRealOut (newRoute, packet->Copy (), ipHeader, m_delay_ms != 0); // see above, its from SendInternal, so if m_delay is != 0 we will already have applied delay
    }
  else
    {
